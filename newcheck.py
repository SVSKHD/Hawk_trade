...
# Function to get the last Friday's closing price using bar data
def get_last_friday_closing(symbol, server_timezone="EET"):
    # Define timezone for the server
    timezone = pytz.timezone(server_timezone)
    ...
    # Set the closing time at 23:59:00 on last Friday
    ...
    # Fetch 1-minute bars from midnight to the market close on Friday
    rates = mt5.copy_rates_range(symbol, mt5.TIMEFRAME_M1, friday_start_time, friday_close_time)
...
def main():
    ...
    # Get the last Friday's closing price
    closing_price, date = get_last_friday_closing(symbol)

    if closing_price:
        print(f"The closing price for {symbol} on {date} was {closing_price}")
    else:
