...
def pip_difference(symbol, start_price, current_price, pip_threshold, thresholds):
    ...
    if abs(pips_from_previous) >= pip_threshold:
        if not thresholds:  # This is the first threshold being reached
            print(f"hello first threshold reached for {symbol} at {current_price} ({pips_from_previous} pips) in the {direction} direction.")
        ...
        print(f"New threshold added for {symbol} at {current_price}. Current list of thresholds: {thresholds}")
    else:
